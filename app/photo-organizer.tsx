...
export default function PhotoOrganizerScreen() {
  ...
  const handleCategorize = (categoryIndex: number, isLeft: boolean) => {
    ...
    if (actualIndex < categories.length) {
      ...
      // Logic to handle batch or single
      if (isMultiSelectMode && batchSelection.size > 0) {
        // Batch categorize - Add all selected images to history
        const processedImages = selectedImages.filter(img => batchSelection.has(img.uri));

        // Add each image to history for DB upload
        const newHistoryItems: HistoryItem[] = processedImages.map(img => ({
  ...
  const handleTrash = () => {
    ...
    if (isMultiSelectMode && batchSelection.size > 0) {
      ...
      // Add each image to history as trash (for undo support)
      const newHistoryItems: HistoryItem[] = processedImages.map(img => ({
        type: 'trash' as const,
        image: img
      }));
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      Alert.alert('완료', `${categorizedItems.length}장의 사진을 저장했습니다!`, [
        { text: '확인', onPress: () => router.back() }
      ]);

    } catch (error) {
      console.error('Photo Save Failed:', error);
      Alert.alert('오류', '사진 저장에 실패했습니다. 다시 시도해주세요.');
    } finally {
      setIsUploading(false);
    }
  };
