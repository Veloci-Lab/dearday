...
/** Private 버킷 업로드 (성공 시 object path 반환) */
async function uploadToStorageReturnPath(
  bucket: string,
  objectPath: string,
  bytes: Uint8Array,
  contentType: string = "image/jpeg",
  cacheControl: string = "public, max-age=31536000, immutable" // 서명URL에도 효력
): Promise<string> {
  ...
  if (error) throw error; // 업로드 실패면 여기서 throw
  return objectPath;       // private이므로 URL이 아닌 경로만 반환
}
...
export default function QuickMemoScreen() {
  ...
  const [remotePaths, setRemotePaths] = useState<{ original: string | null; thumb: string | null }>({
    original: null,
    thumb: null,
  }); // 업로드 완료 후 object path들 (URL 아님)
  ...
  // 업로드 작업 Promise 보관(저장 버튼에서 대기) - 원본+썸네일 경로 반환
  ...
  // 들어오자마자 업로드 시작 (원본 + 썸네일) -> private bucket: "pictures"
  useEffect(() => {
    if (!previewUri || remotePaths.original || remotePaths.thumb || uploading) return;

    const startUpload = async (uri: string): Promise<{ original: string; thumb: string }> => {
      setUploading(true);
      try {
        const BUCKET = "pictures"; // private 버킷
        const ts = Date.now();
        const baseKey = `${profileId}_${ts}`;

        /* === 1) 원본 업로드 (고화질 유지) === */
        const origBytes = await uriToBytes(uri);
        const origPath = `original/photo_${baseKey}.jpg`; // pictures/original/*
        const finalOriginalPath = await uploadToStorageReturnPath(
          BUCKET,
          origPath,
          origBytes,
          "image/jpeg"
        ...
        const thumbPath = `thumb/photo_${baseKey}.jpg`; // pictures/thumb/*
        const finalThumbPath = await uploadToStorageReturnPath(
          BUCKET,
          thumbPath,
          thumbBytes,
          "image/jpeg"
        ...
        setRemotePaths({ original: finalOriginalPath, thumb: finalThumbPath });
        return { original: finalOriginalPath, thumb: finalThumbPath };
      } finally {
        setUploading(false);
      }
    };
  ...
  }, [previewUri, remotePaths.original, remotePaths.thumb, uploading]);
  ...
  // 저장: 업로드 완료 → memories 준비 → memory_entries insert (경로 저장)
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // 업로드 완료 대기
      let finalOriginalPath = remotePaths.original;
      let finalThumbPath = remotePaths.thumb;

      if (!finalOriginalPath || !finalThumbPath) {
        ...
        finalOriginalPath = res.original;
        finalThumbPath = res.thumb;
      }
      if (!finalOriginalPath || !finalThumbPath) {
      ...
      // memory_entries insert (경로 저장!)
      const insertData: any = {
        memory_id,
        entry_index: entryIndex,
        image_path: finalOriginalPath,     // URL 대신 object path
        image_thumb_path: finalThumbPath,  // URL 대신 object path
  ...
  }, [saving, profileId, previewUri, remotePaths.original, remotePaths.thumb, placeName, text, notification_id]);
