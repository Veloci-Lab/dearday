...
// TODAY 아이콘
const ICONS = {
  today: require("@/assets/images/icons/TODAY.png"),
} as const;
...
export default function CalendarScreen() {
  ...
  // TODAY 이미지 크기(비율 유지)
  const TODAY_H = 25;
  const __todaySrc = Image.resolveAssetSource(ICONS.today);
  const TODAY_W = Math.round(TODAY_H * (__todaySrc.width / __todaySrc.height));
        ...
        renderHeader={(date: any) => {
          ...
          return (
            <View style={[styles.headerRow, { paddingHorizontal: CAL_PAD_H, alignSelf: 'stretch', width: '100%' }]}>
              ...
              {/* 오른쪽: TODAY → 이미지 버튼으로 교체 */}
              <Pressable onPress={goThisMonth} hitSlop={8} style={{ paddingHorizontal: 0, paddingVertical: 0 }}>
                <Image
                  source={ICONS.today}
                  style={{ width: TODAY_W, height: TODAY_H, resizeMode: 'contain' }}
                />
              </Pressable>
            </View>
          );
