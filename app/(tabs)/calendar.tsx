...
//       >
//         {/* 캘린더 래퍼 - 스크롤 중에는 스와이프 비활성화 */}
//         <View pointerEvents={isScrolling ? 'none' : 'auto'}>
//           <Calendar
//             ref={calRef}
...
//         {selectedDate && entries.filter((e) => !!e.image_url).length > 0 && (
//           <FlatList
        ...
        <View pointerEvents={isScrolling ? 'none' : 'auto'}>
          <Calendar
            ...
            dayComponent={({ date, state, onPress }) => {
              ...
              return (
                <Pressable onPress={() => { onPress?.(date); onSelectDay(ds); }} style={{ paddingHorizontal: H_GAP / 2 }}>
                  <View style={{ width: cellWidth, height: cellWidth, borderRadius: 10, overflow: 'hidden', position: 'relative' }}>
                    {/* 배경 이미지와 오버레이 */}
                    {hasPhoto && (
                      <>
                        <Image source={{ uri }} style={{ position: 'absolute', width: '100%', height: '100%' }} />
                        <View style={{ position: 'absolute', width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.25)' }} />
                      </>
                    )}
                    {/* 선택된 경우 테두리 - 절대 위치로 변경 */}
                    {isSelected && (
                      <View style={{ 
                        position: 'absolute', 
                        top: 0, 
